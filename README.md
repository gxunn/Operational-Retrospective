...
- 本地开发默认使用 SQLite；生产环境建议使用 Railway Postgres

推荐使用 Railway 部署。生产环境请使用 Railway Postgres 保存核心业务数据，并配合持久化磁盘保存上传原文件和导出报告。Cloudflare 可以用于域名解析、CDN 和 HTTPS，但不能只用 Cloudflare Pages 承载本项目，原因见“Cloudflare 部署说明”。
...
在 Railway 项目里新增 `Postgres` 服务。创建完成后，把应用服务的 `DATABASE_URL` 指向 Railway 自动提供的 Postgres 连接串。

### 5. 添加磁盘

在应用服务里新增一个持久化磁盘，挂载路径填：
...
如果 Railway 给出的连接串是 `postgres://...`，也可以直接填；项目已经支持 Postgres。

如果你要改管理员密码，就把 `ADMIN_PASSWORD` 换成你自己的。

### 7. 如果你已有 SQLite 数据

先把本地 `data/app.db` 迁移到 Railway Postgres：

```bash
export SOURCE_DATABASE_URL=sqlite:///./data/app.db
export TARGET_DATABASE_URL='你的 Railway Postgres 连接串'
python scripts/migrate_sqlite_to_postgres.py
```

如果你还要保留“重新上传原文件”“历史导出 PDF/Word”这类能力，请把本地 `data/uploads` 和 `data/reports` 一并复制到 Railway Volume 的 `/data/uploads` 与 `/data/reports`。
...
以后代码变更后，重新部署一次即可。Postgres 会保留账号、团队、设置、上传记录、日志和业务数据；`/data` 会保留上传原文件与导出报告。
